...
int main(int argc, char** argv) {
    ...
    SolutionBase* sol = new ParallelSolution(batchsize,standby_size,num_agents);
    ...
    for (int batchid=0; batchid<repairbatches.size(); batchid++) {
        ...
        cout << "batch" << curbatch->getBatchId() << " load = " << load <<endl;
        overall_load += load;
        overall_bdwt += curbatch->getBdwt();
    }

    cout << "overall load: " << 1.0*overall_load/ecw << " blocks" << endl;
    cout << "overall bdwt: " << 1.0*overall_bdwt/ecw << " kblocks" << endl;
    return 0;
}
